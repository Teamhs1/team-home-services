...
export default function AdminUsersPage() {
  ...
  // 🔄 Sincronizar roles con Clerk
  const handleSyncRoles = async () => {
    try {
      setSyncing(true);
      toast.info("Syncing roles with Clerk...");
      const res = await fetch("/api/admin/sync-roles", {
        method: "POST",
        credentials: "include",
      });
      ...
      toast.error("❌ Error syncing roles");
    } finally {
      setSyncing(false);
    }
  };
  ...
  const handleRoleChange = async (clerkId, roleValue) => {
    if (!clerkId || !roleValue) {
      console.error("❌ Missing fields:", { clerkId, roleValue });
      toast.error("Missing fields for update.");
      return;
    }

    try {
      console.log("🧩 Role change triggered for:", clerkId, "→", roleValue);
      setChanging(true);
      ...
      if (!res.ok) {
        toast.error(
          `❌ Failed (${res.status}): ${result.message || "Unknown error"}`
        );
