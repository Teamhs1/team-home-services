...
export default function PropertyDetailPage() {
  ...
  const canAccess = role === "admin" || role === "client";
  const canEdit = canAccess; // ambos pueden editar dentro de su scope
  ...
  async function saveName() {
    ...
    try {
      ...
      const res = await fetch(`/api/dashboard/properties/${property.id}`, {
        method: "PATCH",
        headers: {
    ...
    <div className="p-8 pt-[130px] space-y-8 max-w-[1400px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4">
        ...
        {isAdmin ? (
          <div className="mt-2 text-sm text-gray-600">
            ...
            <input
              ...
              onBlur={async () => {
                try {
                  await fetch(`/api/dashboard/properties/${property.id}`, {
                    method: "PATCH",
        ...
        <div className="flex flex-wrap gap-2">
          <button
            onClick={() =>
              router.push(`/dashboard/properties/${property.id}/units/create`)
            }
          ...
          <button
            onClick={() =>
              router.push(`/dashboard/keys/create?property_id=${property.id}`)
            }
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          {isAdmin && (
            <button
              onClick={() =>
                router.push(`/dashboard/properties/${property.id}/set-location`)
              }
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {units.map((u) => (
              <button
                ...
                onClick={() =>
                  router.push(
                    `/dashboard/properties/${property.id}/units/${u.id}`
                  )
                }
            ...
            <table className="min-w-full text-sm">
              ...
              <tbody>
                {keys.map((k) => (
                  <tr
                    key={k.id}
                    onClick={() => router.push(`/dashboard/keys/${k.tag_code}`)}
