...
export async function PATCH(req, { params }) {
  try {
    ...
    // Obtener unit + property para validar acceso
    const { data: unit } = await supabase
      .from("units")
      .select(
        `
...
========================= */
    ...
    if (typeof body.type === "string") {
      updates.type = body.type.trim() === "" ? null : body.type.trim();
    }

    // 🚗 parking
    if (typeof body.parking === "boolean") {
      updates.parking = body.parking;
    }
    // 🚗 parking spots (cantidad de parqueos)
