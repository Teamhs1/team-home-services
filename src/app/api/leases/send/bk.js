...
export async function POST(req) {
  try {
    ...
    // reset (seguro)
    ["S2I", "S2II", "S2III", "S2IV", "S2V"].forEach((f) => {
      try {
    ...
    if (app.property_type === "house") {
      form.getCheckBox("S2I")?.check();
    } else if (app.property_type === "condo") {
      form.getCheckBox("S2II")?.check();
    } else if (app.property_type === "room") {
      form.getCheckBox("S2III")?.check();
    }
